---
title: Linux
summary: "How to install Supersedure Studio on Linux systems"
old_url: "https://docs.supersedurestudio.io/docs/linux"
---


There are several ways to install Supersedure Studio on Linux systems.
...
AppImages can be downloaded and run directly on most Linux distributions without any sort of installation. This is great if you don't have root access, but still want to use Supersedure Studio.

The AppImage distribution of Supersedure Studio provides automatic updates.

Download the latest AppImage [from the Supersedure Studio homepage](https://www.supersedurestudio.io/)
...
Either set the repo up using the code below, or [download the deb file from the latest release](https://github.com/thePeras/supersedure-studio/releases/latest), and it will automatically install the repository on installation.
...
Either set the repo up using the code below, or [download the rpm file from the latest release](https://github.com/thePeras/supersedure-studio/releases/latest), and it will automatically install the repository on installation.
...
Pacman (installed as local packages using `pacman -U`) packages are provided for both x86_64 and ARM64 systems, you can download them from [the latest release](https://github.com/thePeras/supersedure-studio).

There are also several options in the AUR. The Supersedure Studio Team maintain the [supersedure-studio-appimage](https://aur.archlinux.org/packages/supersedure-studio-appimage) and [supersedure-studio-bin](https://aur.archlinux.org/packages/supersedure-studio-bin) packages.

These can be installed with your favourite AUR helper like so:
```bash
paru -S supersedure-studio-appimage
# Or
paru -S supersedure-studio-bin
```

## Flatpak

Flatpak (.flatpak) files are provided separately for both x86_64 and ARM64 systems, you can download them from [the latest release](https://github.com/thePeras/supersedure-studio).
...
You can also install Supersedure Studio through Snapcraft (also part of the Ubuntu Store). Use either the Snap Store link below, or install through the terminal.

!!! warning
    Some features are unavailable in the Snap version of Supersedure Studio due to the security model of Snap packages.
...
View Supersedure in the [Snap Store](https://snapcraft.io/supersedure-studio), or install using the terminal:
...
There are some font-rendering issues with Snaps on the latest version of Gnome with the `snap` version of Supersedure Studio. This is only really visible in the file-select screen. [Hopefully it will be fixed soon](https://forum.snapcraft.io/t/snapped-app-not-loading-fonts-on-fedora-and-arch/12484/66)
...
Run `sudo snap connect supersedure-studio:ssh-keys :ssh-keys`.
...
Supersedure Studio fully supports Wayland (tested on Gnome only) with fractional scaling too.
...
Then restart Supersedure Studio for the change to take effect.
