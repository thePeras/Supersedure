...
Queries and Connections stored in a workspace are only available for use through the Supersedure Studio app, not through a web interface.

![Image Alt Tag](../assets/images/cloud-storage-team-workspaces-28.png)

## Concepts

1. **Workspace** - An online collaborative space for `members` to work on `queries` and `connections`. You get one of these when you create a subscription on the [account dashboard](https://app.supersedurestudio.io)
...
Workspaces are free so long as you maintain an active Supersedure Studio subscription.
...
Click the `(+)` button on the global sidebar to create a workspace. Give it any name you like! If you don't yet have a Supersedure Studio workspace account you will be forwarded to the web interface to create one.
...
Once connected to a workspace connection you'll be able to use Supersedure Studio as normal. The difference is that you can only used saved queries that are part of your cloud workspace.
...
1. Sign in to your workspace in Supersedure Studio
...
1. Sign in to the [Supersedure Studio dashboard][dashboard]
